...
for (const lang of supportedLanguages) {
  ...
  assert(Array.isArray(menu.tabs), `Locale "${lang}" is not on the tabbed menu shape`);
  assert(menu.isPending || menu.tabs.length > 0, `Locale "${lang}" is missing menu tabs`);
  ...
  assert(
    menu.isPending || (filterableTabs.has('cuisine') && filterableTabs.has('pizzas')),
    `Locale "${lang}" is missing expected filterable tabs`,
  );
